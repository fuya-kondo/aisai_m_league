...
## 設計資料・提案資料（クラウド構成時の概算コスト）

> 方針: **固定費ほぼゼロ（従量課金中心）**。  
> ※無料枠の有無、アクセス量、データ転送量、ログ保持期間で上下します。

### 月額見積り（最小利用時の目安）

| 項目 | サービス | 最小利用時の月額レンジ（目安） | 補足 |
| --- | --- | --- | --- |
| フロント配信 | CloudFront | $0.5〜$3 | 低トラフィック配信を想定（リクエスト課金 + 転送料） |
| API | API Gateway (HTTP API) | $0.2〜$2 | 少量APIコールを想定 |
| バックエンド | Lambda | $0〜$1 | 無料枠内〜ごく少量実行 |
| データ保存 | DynamoDB (オンデマンド) | $0.2〜$3 | 少量Read/Write + 小容量ストレージ |
| 静的ストレージ | S3 | $0.1〜$1.5 | 少量オブジェクト保管 + 少量GET/PUT |
| DNS | Route 53 | $0.5〜$1 | Hosted Zone + 少量クエリ |
| TLS証明書 | ACM（証明書運用） | $0 | 公開証明書を利用する前提 |
| ログ保管 | CloudWatch Logs / S3 | $0.3〜$3 | 少量ログ取り込み + 短〜中期間保存 |

### 合計イメージ

- **最小構成の目安**: **$1.8〜$14.5 / 月 程度**
- 上記より、運用方針は **固定費ほぼゼロ（従量課金中心）** とする。
- アクセス増加時は CloudFront 転送量、API Gateway リクエスト数、ログ保管量が主な増分要因。
